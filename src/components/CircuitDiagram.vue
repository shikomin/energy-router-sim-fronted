...
const getDevicePower = (deviceId: string): number => {
  ...
  if (deviceId.includes("mppt")) {
    return (
      ((telemetry.pv1Power || 0) +
        ...
        (telemetry.pv3Power || 0) +
        (telemetry.pv4Power || 0)) /
      10
    );
  }
  ...
  if (deviceId.includes("bms")) {
    return (
      ((-1 * telemetry.clusterCurrent * telemetry.clusterVoltage) / 1000 || 0) /
      10
    );
  }
...
const calculateLayout = () => {
  ...
  // 直流负载 接线
  lines.push({
    fromX: dcBusStartX + busLength - 70,
    fromY: pcsY + deviceHeight / 2,
    toX: dcLoadX,
...
const drawConnection = (
  ...
  let { fromX, fromY, toX, toY, power, type } = line;
      ...
      for (let i = 1; i <= pulseCount; i++) {
        ...
        if (offset < length * 0.8) {
          ...
          const distanceFromCenter =
            Math.abs(offset - length / 2) / (length / 2);
    ...
    <div class="canvas-container">
      ...
      >
        ...
        <div v-if="hoveredDeviceData.runtime" class="tooltip-runtime">
          <div
            v-for="(value, key) in hoveredDeviceData.runtime.telemetry"
            :key="key"
            class="tooltip-row"
          >
            <span class="tooltip-key">{{ key }}:</span>
            <span class="tooltip-value">{{
              typeof value === "number" ? (value / 10).toFixed(2) : value
            }}</span>
          </div>
        </div>
